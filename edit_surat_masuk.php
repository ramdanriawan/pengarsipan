<?php
    ...
    if(empty($_SESSION['admin'])){
                    ...
                    if(!preg_match("/^[a-zA-Z0-9.\/ -]*$/", $no_surat)){
                        ...
                        if(!preg_match("/^[a-zA-Z0-9.,() \/ -]*$/", $asal_surat)){
                            ...
                            if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $isi)){
                                ...
                                if(!preg_match("/^[a-zA-Z0-9., ]*$/", $ntgl_diterima)){
                                    $_SESSION['etgl_diterima'] = 'Form Tgl Diterima hanya boleh mengandung angka dan minus(-)';
                                    echo '<script language="javascript">window.history.back();</script>';
                                } else {

                                    if(!preg_match("/^[a-zA-Z0-9., -]*$/", $input)){
                                        $_SESSION['einput'] = 'Form Input hanya boleh mengandung karakter huruf, angka, spasi, titik(.) dan koma(,) dan minus (-)';
                                        echo '<script language="javascript">window.history.back();</script>';
                                    } else {
                                        ...
                                        if(!preg_match("/^[0-9.-]*$/", $tgl_surat)){
                                            ...
                                            if($file != ""){
                                                ...
                                                if(in_array($eks, $ekstensi) == true){
                                                    if($ukuran < 2300000){
                                                        ...
                                                        if(!empty($file)){
                                                            ...
                                                            $query = mysqli_query($config, "UPDATE tbl_surat_masuk SET no_agenda='$no_agenda',no_surat='$no_surat',asal_surat='$asal_surat',isi='$isi',tgl_diterima='$ntgl_diterima',input='$input',tgl_surat='$tgl_surat',file='$nfile',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");
                                                ...
                                                $query = mysqli_query($config, "UPDATE tbl_surat_masuk SET no_agenda='$no_agenda',no_surat='$no_surat',asal_surat='$asal_surat',isi='$isi',tgl_diterima='$tgl_diterima',input='$input',tgl_surat='$tgl_surat',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");
        ...
        $query = mysqli_query($config, "SELECT id_surat, no_agenda, no_surat, asal_surat, isi, tgl_diterima, input, tgl_surat, file, keterangan, id_user FROM tbl_surat_masuk WHERE id_surat='$id_surat'");
        list($id_surat, $no_agenda, $no_surat, $asal_surat, $isi, $tgl_diterima, $input, $tgl_surat, $file, $keterangan, $id_user) = mysqli_fetch_array($query);
                    ...
                    <div class="row">
                        <div class="input-field col s6 tooltipped" data-position="top" data-tooltip="Isi dengan angka">
						 ...
						 <div class="input-field col s6">
                            ...
                            <input id="tgl_diterima" type="text" name="tgl_diterima" class="datepicker2" value="<?php echo $tgl_diterima ;?>" required>
                                <?php
                                    if(isset($_SESSION['etgl_diterima'])){
                                        $etgl_diterima = $_SESSION['etgl_diterima'];
                                        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$etgl_diterima.'</div>';
                                        unset($_SESSION['etgl_diterima']);
                                    }
                                ?>
                        ...
                        <div class="input-field col s6 tooltipped" data-position="top" data-tooltip="Isi dengan huruf atau angka">
                            ...
                            <input id="indeks" type="text" class="validate" name="input" value="<?php echo $input ;?>" required>
                                <?php
                                    if(isset($_SESSION['einput'])){
                                        $einput = $_SESSION['einput'];
                                        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$einput.'</div>';
                                        unset($_SESSION['einput']);
                                    }
                                ?>
                            <label for="input">Di Inputkan Oleh</label>
                        </div>
